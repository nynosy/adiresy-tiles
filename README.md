...
Off-device build pipeline that produces Madagascar `.pmtiles` (national only, in two quality tiers, plus a buildings overlay, a POI overlay, and an administrative boundaries overlay) and a `manifest.json`, published as GitHub Release assets on a quarterly schedule.

## Quality tiers

The national base map is built at two zoom levels, so users can pick download size vs. map detail. Each tier also has a matching **buildings overlay** — a separate `.pmtiles` file with denser building footprint data (see below) — this is the app's main feature, always downloaded alongside the base map, not an optional extra — and a **POI overlay** (schools, hospitals, shops, etc. — see below) that adds only a few hundred KB to MB:
...
(Sizes measured directly from a local build/extract of the national extent. A third tier, Z14/Detailed, and a per-region download split were both built and evaluated earlier but dropped — see `docs/TileGen-Implementation-Spec.md`'s "Current State" note for why.)
...
A single small `boundaries.pmtiles` (**13.3 MB**, not split by tier — the whole country fits comfortably in one file) carries region/district/commune/fokontany boundary lines, matching what the live [adiresy.mg](https://adiresy.mg/) site renders. Sourced from BNGRC/OCHA's COD-AB dataset (see below), built with `tippecanoe` from the official Shapefile — there's no pre-tiled version like VIDA provides for buildings, so this repo does the conversion itself (`scripts/build-boundaries.sh`). Independent of the base map tiers and unaffected by anything else in this README — it's a fixed national dataset showing Madagascar's *current* administrative divisions (24 regions, not the legacy 6 provinces — see `scripts/build-boundaries.sh`'s `admLevel` correction below).

Consumed by the `adiresy-mobile` Android app via `TileDownloadWorker`, which downloads the national bundle and quality tier the user picks and verifies it against the manifest's SHA-256 before use.
...
# Buildings overlay, one call per tier
bash scripts/extract-buildings.sh buildings-madagascar-z12.pmtiles "" 12
bash scripts/extract-buildings.sh buildings-madagascar-z13.pmtiles "" 13

# POI overlay, one call per tier -- fixed min_zoom=12, visible starting at the Overview tier
# (see "POI overlay" above)
bash scripts/build-poi-overlay.sh madagascar "" poi-madagascar-z12.pmtiles 12
bash scripts/build-poi-overlay.sh madagascar "" poi-madagascar-z13.pmtiles 13

# Admin boundaries overlay -- single file, no tiers
bash scripts/build-boundaries.sh boundaries.pmtiles

# Manifest (after building whichever files you want to include)
...
The first `build.sh` run downloads `planetiler.jar` and the Geofabrik Madagascar extract into the working directory; subsequent runs in the same directory reuse both, so building both tiers only pays the download cost once. `build-poi-overlay.sh` reuses the same `planetiler.jar`, just via a different entry point (a custom schema, not the default OpenMapTiles profile), so it doesn't re-download anything either. `extract-buildings.sh` pulls directly from VIDA's remote archive via HTTP range requests — no full-archive download needed, but each tier extraction does transfer its own share (see the size table above). `build-boundaries.sh` downloads the ~63 MB BNGRC/OCHA shapefile fresh each run (it's small and the source is effectively static, so this isn't worth caching).

## Release process

Releases are built and published automatically by `.github/workflows/build-tiles.yml` on a quarterly schedule (1 Jan/Apr/Jul/Oct), or on demand via the workflow's manual trigger. Each release is tagged `data-{YYYY}-Q{N}` and contains the national `.pmtiles` files in both quality tiers, a matching buildings overlay and POI overlay `.pmtiles` for each tier, a single `boundaries.pmtiles`, their `.sha256` checksums, and `manifest.json` — 9 tile files total. The app always fetches the manifest from the release's `/releases/latest/download/` URL, so it never needs a hardcoded tag.
