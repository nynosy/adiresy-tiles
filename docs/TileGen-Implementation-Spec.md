...
**Decision (resolves TG-2):** rather than picking one fixed maxzoom, the pipeline builds **two quality tiers**, national only, so the app can offer users a size/detail tradeoff instead of a single fixed download:
...
These are real measurements from a local build of the national extract, not estimates. A third **Detailed** tier (Z14, ~302 MB) and a per-province/per-region download split were both built and evaluated, but dropped before ever shipping — see the History appendix (§13) for why. An experiment restricting Planetiler's `--languages` flag (to cut down the ~90 default name-translation languages) was also tried against the Z14 build and made a negligible difference (301.8 MB → 301.4 MB, ~0.1%) — geometry, not name text, dominates file size, so that lever isn't worth pursuing regardless of tier count.
...
area: madagascar
minzoom: 0
maxzoom: 13        # Standard tier -- no Z14/Detailed tier, see docs/TileGen-Implementation-Spec.md's "Current State" note.
download: true      # Fetches the Geofabrik extract automatically; cached under sources/
```

## 4. `scripts/build.sh`

Builds the national base map at either quality tier — `maxzoom` is a parameter rather than hardcoded. Reruns within the same working directory reuse the cached Geofabrik download instead of re-fetching it, so building both tiers only pays the download cost once. The `bounds` argument still accepts either a bbox string or a `.poly` polygon path, clipping with Planetiler's `--polygon=` in the latter case, but the current national-only pipeline never calls it with one — `.github/workflows/build-tiles.yml` always passes `bounds=""`. The parameter is kept generic rather than removed since it costs nothing to leave in and a per-region download split was built and evaluated once already (see the History appendix). **Note:** `--polygon=` takes the Osmosis polygon filter text format, not GeoJSON — this was confirmed the hard way during that region-split work (see the appendix) after first assuming GeoJSON would work.
...
**Verified locally:** national Z12 (`madagascar-z12.pmtiles`, 42M — matches §3's ~43MB) and Z13 both ran end-to-end. `--polygon=`'s `.poly`-not-GeoJSON requirement (noted above) was caught for real during the since-abandoned province/region-split work — see the History appendix for the failure mode (`FileFormatException: File ends before end of polygon`) and how it was fixed there.
...
| Tier | Base map | + Buildings overlay | Total |
|---|---|---|---|
| Z12 | 43 MB | +153 MB | 196 MB |
| Z13 | 102 MB | +327 MB | 429 MB |

(A Z14/Detailed tier and a per-province/per-region split were both measured during evaluation — Z14 national was 302 MB base + 694 MB buildings; Antananarivo, the largest of the 6 legacy provinces, was 71.5 MB + 188 MB at Z14, Mahajanga the smallest at 21.9 MB + 82 MB — but neither shipped. See the History appendix.)

Given this cost, the decision was to ship the buildings overlay at **both quality tiers**, national only (2 buildings files, matching the 2 base-map files — 4 tile files total per release), rather than restricting it to one tier. This was a deliberate choice given the size tradeoff, not a default — revisit if release size or CI bandwidth becomes a problem (see TG-12).

**File naming convention:** `buildings-madagascar-{tier}.pmtiles` — mirrors the base map's `madagascar-{tier}.pmtiles` but with a `buildings-` prefix, so release-asset globs stay disjoint (see §9).
...
**Current national-only matrix verified locally:** both tiers extracted (`buildings-madagascar-z12.pmtiles`, `buildings-madagascar-z13.pmtiles`), `pmtiles show` valid, `.sha256` checksums verified.

**The transient-500 retry logic in the script above isn't precautionary — it was needed for real,** confirmed during the since-abandoned province-split evaluation (History appendix), when this script ran against a 21-file matrix (national + 6 provinces × 3 tiers). The first unattended run of all 21 extractions failed twice (once on Antananarivo Z12, once on Toliara Z12), both times with a `HTTP error: 500` from the upstream server at ~99% of the transfer, leaving a truncated/corrupt output file (`pmtiles show` failed with "magic number not detected" until the corrupt file was deleted and the extraction retried). A subsequent run with retry logic hit **5 more transient failures** across the remaining files, all recovered within 1–2 retries — roughly a ~25% per-file failure rate. This confirms TG-11 (VIDA as an external dependency) is a real, recurring reliability concern, not theoretical, and the retry logic stays regardless of how many files the pipeline extracts per release.
...
The underlying OSM data and Planetiler's extraction are both fine — this is OpenMapTiles' built-in vector-tile schema deliberately gating most POI classes to `min_zoom: 14` to control tile size/clutter, undocumented anywhere in this spec until now. Since §3 recommends the Standard (Z13) tier as the default, most users would see **no** schools, hospitals, pharmacies, or shops at all. (The Detailed/Z14 row above predates the pipeline dropping that tier — see the "Current State" note — but the underlying problem it illustrates, OpenMapTiles gating POI classes to z14 regardless of what tiers we ship, is unchanged and is exactly what the fix below addresses.)
...
- `type: match_key` / `type: match_value` attribute types (used for a `class`/`subclass` pair, mirroring OpenMapTiles' own convention) produce exactly the values inferred from the JSON schema (`planetiler.schema.json`) — e.g. for `amenity=school`, `match_key="amenity"`, `match_value="school"`. (These map onto `class`/`subclass` in reverse — see the attributes block below.)
- `--bounds=` and `--polygon=` both work identically to `scripts/build.sh`'s invocation, since both entry points share the same `PlanetilerConfig` argument parsing — confirmed against `.poly` files generated during the since-abandoned region-split work (History appendix); unused by the current national-only pipeline.
...
**`config/poi-overlay.yml`:** a curated set of `amenity`/`shop`/`healthcare`/`office`/`tourism`/`leisure`/`railway` tag values (chosen to cover the same real-world categories observed in the base map's own Z14 `poi` layer — healthcare, education, government/civic, food, shops, transit, lodging), all emitted to a single `poi` layer at a fixed `min_zoom: 12` — meaning they now appear starting at the *Overview* tier, the same as every other tier, resolving the original complaint directly. `class`/`subclass` attributes come from raw OSM tag key/value (not OpenMapTiles' internal normalized taxonomy — e.g. `class` may say `supermarket` where OpenMapTiles would say `grocery` — simpler and more transparent, at the cost of not being byte-for-byte identical to the base map's own category strings). Note `class` takes the OSM tag's *value* (the specific category, e.g. `supermarket`) and `subclass` takes its *key* (the broad group, e.g. `shop`) — the reverse of what the field names alone suggest; see the attributes block below for why.
      ...
      - source: osm
        ...
        include_when:
          ...
          railway:
            ...
            - halt
        attributes:
          # Planetiler's match_key/match_value are named from the OSM tag's
          # own perspective, not OpenMapTiles' schema convention: match_key
          # is the broad tag KEY that satisfied include_when (amenity, shop,
          # healthcare, office, tourism, leisure, railway -- 7 values), and
          # match_value is that key's specific VALUE (hospital, restaurant,
          # supermarket, ...). Consumers (e.g. adiresy-mobile's StyleLoader,
          # which matches specific categories like "hospital"/"restaurant")
          # expect OpenMapTiles-style class/subclass instead, where class is
          # the specific category -- so class here takes match_value and
          # subclass takes match_key, the reverse of what the names suggest.
          - key: class
            type: match_value
          - key: subclass
            type: match_key
...
**Verified locally:** the real committed script (not just scratch experiments) built successfully — national, both tiers (644K/2.0M, tiny relative to the base map's 43–102MB), valid `pmtiles show` output, `.sha256` checksums generated. `generate-manifest.py`'s `poi` section (see §9) tested with a dry run and produces the expected nested structure. (A `--polygon=`-clipped province test — Mahajanga, class distribution 5,478 amenity / 2,588 shop / 973 office / 711 tourism / 75 leisure / 50 railway / 4 healthcare — was also run during the since-abandoned region-split evaluation; see the History appendix.)

**File naming convention:** `poi-madagascar-{tier}.pmtiles` — same `poi-` prefix pattern as `buildings-`, keeping release-asset globs disjoint (see §10).
...
**Historical note:** this dataset also carries an `OLD_PROVIN` field mapping every feature to one of Madagascar's 6 legacy "faritany" provinces. It was once used to derive an exact-geometry province download split (dissolving ADM1 polygons by `OLD_PROVIN`), later migrated to an ADM2 (district) dissolve for a 24-region split — neither shipped. `OLD_PROVIN` is **not used anywhere in the current pipeline**. See the History appendix for the full story.
...
**Zoom scheme used:** region (level 1) and the coastline/external layer (level 99) from z0; district (level 2) from z4; commune (level 3) from z7; fokontany (level 4) from z9. Tileset built z0–z12 (matches the Overview/Standard tier ceiling; MapLibre will overzoom these simple line tiles fine beyond z12, same as any raster/vector source displayed past its native maxzoom).
...
> **⚠️ Not part of the current pipeline.** An earlier revision of this document had a full
> `scripts/generate-province-polygons.sh` (and later `generate-region-polygons.sh`) here, used to
> derive exact per-province/per-region download-split geometry from this same BNGRC/OCHA dataset's
> `OLD_PROVIN` field / ADM2 layer. Neither script exists in the repo — the download-split idea was
> evaluated and dropped before shipping (see the "Current State" note at the top of this document).
> The real bugs found while building those scripts (GDAL's `-nlt PROMOTE_TO_MULTI` flag name,
> `OLD_PROVIN`'s "Toliary" vs. "Toliara" spelling, Planetiler's `.poly`-not-GeoJSON requirement) are
> preserved in the History appendix since they're non-obvious and could resurface if this kind of
> geometry work is ever needed again — for the boundaries overlay itself or otherwise.

## 8. `scripts/generate-manifest.py`

Scans the current directory for build outputs and writes `manifest.json`. File naming convention: `madagascar-{tier}.pmtiles` / `buildings-madagascar-{tier}.pmtiles` / `poi-madagascar-{tier}.pmtiles`, national only, where `tier` is `z12`/`z13`. No region or province split of any kind — see the "Current State" note at the top of this document and the History appendix for the two prior designs (`files.provinces` / `files.regions`) that were tried and dropped before this script reached its current, simpler national-only shape.
...
**Decision (resolves an ambiguity in the original plan):** an early draft of this spec showed a `"provinces"` key but didn't say whether it nests under `"files"` or sits alongside it. Two later revisions of the shipped script actually nested it (`files.provinces`, then `files.regions` after the region-split migration — see the History appendix), before the whole per-area split was dropped and `files` collapsed back to just `national`. Formal schema below.

## 9. `manifest.json` schema

> **Updated to match current behavior** (unlike most of this document — see the top-of-file
> banner): national-only, no `regions` key anywhere, tiers limited to `z12`/`z13`.

`national` maps to an object keyed by quality tier (`z12`/`z13`) instead of directly to a single file entry. `buildings` and `poi` mirror `files.national`'s shape exactly, as top-level siblings. `buildings` and `boundaries` additionally carry a `dataset_date` — the upstream snapshot date, distinct from `osm_extract_date` since neither refreshes on our quarterly cadence (resolves TG-12/TG-14). `poi` does **not** — it's derived from the same OSM extract as the base map every build, so `osm_extract_date` already covers it (see §6).
...
Example `files.national` and `buildings` (real values, from a local build/extract):
...
"buildings": {
  "national": {
    ...
    "z13": { "filename": "buildings-madagascar-z13.pmtiles", "url": "…", "size_bytes": 327000000, "sha256": "…" }
  },
  "dataset_date": "2024-08-27"
}
```

Example `poi` (real shape, verified with a dry run against stub files, see §11):

```json
"poi": {
  "national": {
    "z12": { "filename": "poi-madagascar-z12.pmtiles", "url": "…", "size_bytes": 654000, "sha256": "…" },
    "z13": { "filename": "poi-madagascar-z13.pmtiles", "url": "…", "size_bytes": 2000000, "sha256": "…" }
  }
}
```
...
The Android app (`TileDownloadWorker`) should validate against this shape defensively (missing `national`, malformed `sha256`) before starting a download — the manifest is fetched over the network and shouldn't be trusted blindly. This is a **breaking schema change** from the single-file-per-region shape in an earlier draft of this spec — flag to the `adiresy-mobile` team if any client code was already written against that shape. `buildings`, `poi`, and `boundaries` are all **additive** changes on top of that — a manifest consumer that doesn't yet know about them can safely ignore those keys. A prior revision of this schema also had `files.regions`/`buildings.regions`/`poi.regions` siblings for a per-region download split; that never shipped and those keys do not exist in the current schema — see the History appendix.

## 10. `.github/workflows/build-tiles.yml`

Single job, sequential builds — 2 quality tiers × (base map + buildings overlay + POI overlay) = 6 tile-producing steps, national only, plus the single boundaries build. Reuses Planetiler's Geofabrik download cache across the base-map and POI-overlay builds within the same job, so the country extract is only downloaded once per run regardless of tier count. The buildings extractions pull directly from VIDA's remote archive instead (see §5).

A per-province and later per-region download split were both built and measured in earlier revisions of this workflow (a real 6-province run took 47m15s total, of which the province loop alone was 37m20s; a 24-region loop was projected at ≈2h39m) — see the History appendix. Neither shipped; the current national-only workflow runs a fraction of that.
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build national tiles (Z12/Z13 only)
        run: |
          for zoom in 12 13; do
            bash scripts/build.sh madagascar "" "madagascar-z${zoom}.pmtiles" "$zoom"
          done

      - name: Extract national buildings overlay (Z12/Z13 only)
        run: |
          for zoom in 12 13; do
            bash scripts/extract-buildings.sh "buildings-madagascar-z${zoom}.pmtiles" "" "$zoom"
          done

      - name: Build national POI overlay (Z12/Z13 only)
        run: |
          for zoom in 12 13; do
...
The glob prefixes (`madagascar-z*`, `buildings-*`, `poi-*`, plus the standalone `boundaries.pmtiles`) are disjoint by construction — buildings files are named `buildings-madagascar-*` and POI files `poi-madagascar-*`, so each only matches its own glob, never the others. Earlier revisions of this workflow also had a `region-*`/`province-*` glob for the per-area download split; that step and glob no longer exist — see the History appendix.
...
- **Manifest:** run `generate-manifest.py` locally and validate the output against the JSON Schema in §9 (e.g. with `check-jsonschema`) — tested against real national base map, buildings, POI overlay, and boundaries files.
- **CI dry run:** trigger `workflow_dispatch` once manually before relying on the cron schedule, to confirm the release is created correctly, all tile-file asset URLs resolve (6 tile files + boundaries + manifest + checksums), and the `pmtiles`/`gdal-bin`/`tippecanoe` install step works on `ubuntu-latest`.
...
A per-province, then per-region download split (T5's original scope) was designed, built, and measured but never shipped — see the History appendix.

---

## 13. History appendix: the province/region download-split that was tried and dropped

> **⚠️ Nothing in this section describes current behavior.** None of it is implemented — see the
> "Current State" banner at the top of this document and
> `docs/National-Only-Simplification-Implementation-Spec.md` in `adiresy-mobile` for what actually
> shipped (national-only, Z12/Z13-only). This section exists purely as a record of two rounds of
> real investigative work — verified BNGRC/OCHA shapefile field spellings, real bugs found and
> fixed, real CI timing measurements — in case per-area splitting is ever revisited. None of the
> scripts described below (`generate-province-polygons.sh`, `generate-region-polygons.sh`) exist
> in the repo; neither does the `region-`/`province-` naming or `files.regions`/`files.provinces`
> manifest shape they produced.

**The idea:** rather than one national `.pmtiles` per tier, split the base map, buildings overlay, and POI overlay into smaller per-area downloads, so users on a limited connection wouldn't need to fetch the whole country. Two versions of this were built, in order:

**v1 — 6 legacy provinces (*faritany*).** BNGRC/OCHA's ADM1 polygon layer carries an `OLD_PROVIN` field mapping exactly onto Madagascar's 6 pre-2004 provinces. `scripts/generate-province-polygons.sh` dissolved ADM1 by `OLD_PROVIN` (`ogr2ogr … -dialect sqlite -sql "SELECT OLD_PROVIN, ST_Union(geometry) … GROUP BY OLD_PROVIN"`), writing one `province-<name>.poly` (Osmosis polygon filter format) plus a `province-bounds.json` bbox index. `scripts/build.sh` and `scripts/build-poi-overlay.sh` accepted the `.poly` path via `--polygon=`; `scripts/extract-buildings.sh` used the bbox via `--bbox=` (`pmtiles extract` has no polygon-clip option). Real bugs found and fixed while building this:
...
2. BNGRC/OCHA's `OLD_PROVIN` attribute spells the sixth province **"Toliary"**, not "Toliara" — the initial name mapping used the latter, which would have silently dropped that province's polygon (caught only because the script's own `missing`-values check exits with an error rather than dropping silently).
3. Planetiler's `--polygon=` does not accept GeoJSON, only the Osmosis `.poly` text format — the first attempt with a `.geojson` path failed with `FileFormatException: File ends before end of polygon`.

All three fixed, then verified end-to-end against the real BNGRC/OCHA zip: 6 valid `.poly` files plus `province-bounds.json`; a `--polygon=`-clipped Antananarivo build and a `--bbox=`-clipped buildings extraction both produced bounds exactly matching that province's computed bbox. This 6-province × 3-tier matrix (base map + buildings + POI = 63 tile-producing steps) was fully built and measured: a real CI run took **47m15s** total, of which the province loop (54 tile-producing invocations) was **37m20s**. `build.sh`'s Planetiler run costs a near-constant ~80–100s per invocation **regardless of output size** (Antananarivo Z12, 8.5 MB, took ~83s; Toamasina Z12, 5.7 MB, took ~82s) — Planetiler pays to scan/decode the whole country extract every invocation, so cost scales with invocation count, not land area. The buildings-overlay extraction (VIDA, over HTTP) also surfaced a real reliability issue at this scale: the first unattended 21-extraction run (national + 6 provinces × 3 tiers) hit 2 transient `HTTP error: 500`s near the end of a transfer, and a retry-equipped rerun hit 5 more — roughly a ~25% per-file failure rate, which is why `extract-buildings.sh`'s 5-attempt retry loop exists.

**v2 — migration to 24 current regions (2026-07).** The 6-province split followed an administrative level abolished in 2004; the rest of the repo (the boundaries overlay) already targeted the current 24-region hierarchy. Checked (2026-07-13): no updated authoritative region-polygon source exists — HDX's COD-AB shapefile still reports only 22 ADM1 features (frozen at the 2004 split, same root cause as the boundary-line admLevel drift, GH issue #2), geoBoundaries also reports 22, and OSM has no reliable region-level coverage for Madagascar. Fix: dissolve from ADM2 (district), not ADM1 — district *geometry* wasn't affected by the 2021 Vatovavy/Fitovinany split or the 2023/2025 Ambatosoa/Analanjirofo split, only which region 9 specific districts are grouped under, so `scripts/generate-region-polygons.sh` dissolved ADM2 by a corrected region label, reusing the exact district P-code sets (`VATOVAVY_DISTRICTS`, `FITOVINANY_DISTRICTS`, `AMBATOSOA_DISTRICTS`) `scripts/build-boundaries.sh` already validated for the boundary-line admLevel fix (§7). Real spelling mismatches found by checking `ogrinfo` directly rather than assuming, the same class of bug as v1's "Toliary" vs. "Toliara":

| Assumed | Real `ADM1_EN` |
|---|---|
| Alaotra-Mangoro | **Alaotra Mangoro** (space, not hyphen) |
| Amoron'i Mania | **Amoron I Mania** (no apostrophe) |
| Atsimo-Andrefana | **Atsimo Andrefana** (space, not hyphen) |
| Atsimo-Atsinanana | **Atsimo Atsinanana** (space, not hyphen) |
| Matsiatra Ambony | **Haute Matsiatra** (different name entirely) |

One more real bug, not caught until an end-to-end run: `ogr2ogr`'s GeoJSON output carries the **source** shapefile's own layer name through into the output's top-level `"name"` field — it is not derived from the output filename. The dissolve SQL's `FROM adm2_corrected` (matching the intermediate output *filename*) therefore failed with `no such table: adm2_corrected`; fixed by pinning the layer name explicitly with `-nln adm2` on the initial conversion and querying `FROM adm2` instead. Verified end-to-end against the real zip: produced 24 valid `region-<name>.poly` files and a 24-entry `region-bounds.json`; smallest outputs were `region-fitovinany.poly`/`region-vatovavy.poly` (both 3-district slices of the old combined region) — expected, and a sanity check that the split was real and non-degenerate. `--polygon=`-clipped POI and base-map builds against a sample region (Mahajanga, under the old naming) were also re-verified: 5,478 amenity / 2,588 shop / 973 office / 711 tourism / 75 leisure / 50 railway / 4 healthcare POI features, matching class-distribution expectations.

Naming would have changed `province-{name}-{tier}.pmtiles` → `region-{name}-{tier}.pmtiles` (and the `buildings-`/`poi-` prefixed variants) throughout `build.sh`'s callers, the workflow, and `generate-manifest.py`; `manifest.json`'s `files.provinces`/`buildings.provinces`/`poi.provinces` would have become `files.regions`/`buildings.regions`/`poi.regions`. CI cost was projected (not run to completion) at 24 regions × 3 tiers × 3 file types = 216 invocations ≈ 37m20s × 4 ≈ 149 min, plus ~10 min for the unaffected rest of the pipeline ≈ **2h39m total** — comfortably inside GitHub Actions' 6-hour job limit, so no CI-scaling mitigation (matrix parallelization, dropping a tier) was judged necessary.

**Why it was dropped:** before the 24-region rebuild and the `adiresy-mobile` client-side changes shipped, the whole per-area download-split approach — along with the Z14/Detailed tier — was reconsidered and dropped in favor of the simpler national-only, Z12/Z13 design this document's main body now describes. See `docs/National-Only-Simplification-Implementation-Spec.md` in `adiresy-mobile` for that decision.

**Not yet done as of this section being written:** the actual `workflow_dispatch` trigger of the updated pipeline (which overwrites the current `data-2026-Q3` release — a production action requiring separate sign-off before running) and the corresponding `adiresy-mobile`-side code changes (tracked in that repo's own migration spec). Real per-region `size_bytes` in the size tables throughout §3/§5/§9's examples remain the pre-migration province-level figures until that rebuild runs.
